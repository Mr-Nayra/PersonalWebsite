# Website Pages Checklist

Here is a complete checklist of all the pages on the website:

### Main Pages
- [ ] `/` (Home)
- [ ] `/about`
- [ ] `/about/beyond-work`
- [ ] `/contact`
- [ ] `/faq`
- [ ] `/process`
- [ ] `/testimonials`

### Services (6 pages)
- [ ] `/services` (Overview)
- [ ] `/services/ai-automation-consulting`
- [ ] `/services/ai-integration` (AI Integration & Agentic Workflows)
- [ ] `/services/ai-voice-agents`
- [ ] `/services/seo-services`
- [ ] `/services/web-development`
- [ ] `/services/saas-development`

### Products (4 pages)
- [ ] `/products`
- [ ] `/products/speclens-ai`
- [ ] `/products/mediamitra`
- [ ] `/products/geo-checker`

### Case Studies (7 pages)
- [ ] `/case-studies`
- [ ] `/case-studies/seo-internal-linking-ai-agent`
- [ ] `/case-studies/autonomous-inbound-email-agent`
- [ ] `/case-studies/speclens-ai-procurement-saas`
- [ ] `/case-studies/photoshoprequest-ai-editor`
- [ ] `/case-studies/healthcare-rcm-automation`
- [ ] `/case-studies/combinehealth-seo-redesign`

### Content & Resources
- [ ] `/blog` (Overview)
- [ ] `/resources` (Overview)
- [ ] `/resources/ai-automation-guide-for-business`
- [ ] `/resources/geo-optimisation-guide`
- [ ] `/resources/technical-seo-audit-checklist`
- [ ] `/resources/ai-voice-agents-explainer`
- [ ] `/resources/nextjs-saas-architecture`

### Glossary (52 terms)
- [ ] `/glossary` (Overview)

**AI & Automation (18)**
- [ ] `/glossary/agentic-ai`
- [ ] `/glossary/ai-agent`
- [ ] `/glossary/ai-orchestration`
- [ ] `/glossary/chain-of-thought`
- [ ] `/glossary/context-window`
- [ ] `/glossary/embeddings`
- [ ] `/glossary/fine-tuning`
- [ ] `/glossary/function-calling`
- [ ] `/glossary/hallucination`
- [ ] `/glossary/langgraph`
- [ ] `/glossary/large-language-model`
- [ ] `/glossary/model-context-protocol`
- [ ] `/glossary/natural-language-processing`
- [ ] `/glossary/prompt-engineering`
- [ ] `/glossary/rag`
- [ ] `/glossary/rcm`
- [ ] `/glossary/vector-database`
- [ ] `/glossary/zero-shot-prompting`

**SEO & GEO (18)**
- [ ] `/glossary/ai-overviews`
- [ ] `/glossary/backlink`
- [ ] `/glossary/canonical-tag`
- [ ] `/glossary/core-web-vitals`
- [ ] `/glossary/crawl-budget`
- [ ] `/glossary/domain-authority`
- [ ] `/glossary/e-e-a-t`
- [ ] `/glossary/entity-seo`
- [ ] `/glossary/geo`
- [ ] `/glossary/internal-linking`
- [ ] `/glossary/knowledge-graph`
- [ ] `/glossary/llms-txt`
- [ ] `/glossary/programmatic-seo`
- [ ] `/glossary/robots-txt`
- [ ] `/glossary/search-intent`
- [ ] `/glossary/semantic-seo`
- [ ] `/glossary/structured-data`
- [ ] `/glossary/technical-seo`

**AI Voice (5)**
- [ ] `/glossary/ivr`
- [ ] `/glossary/latency`
- [ ] `/glossary/speech-to-text`
- [ ] `/glossary/text-to-speech`
- [ ] `/glossary/voice-cloning`

**Web & SaaS (11)**
- [ ] `/glossary/api`
- [ ] `/glossary/ci-cd`
- [ ] `/glossary/headless-architecture`
- [ ] `/glossary/microservices`
- [ ] `/glossary/multi-tenant-saas`
- [ ] `/glossary/mvp`
- [ ] `/glossary/saas-metrics`
- [ ] `/glossary/server-side-rendering`
- [ ] `/glossary/technical-co-founder`
- [ ] `/glossary/webhook`
- [ ] `/glossary/websocket`

### Industries & Locations
- [ ] `/industries/saas`
- [ ] `/industries/edtech`
- [ ] `/industries/healthcare`
- [ ] `/industries/ecommerce`
- [ ] `/industries/startups`
- [ ] `/location/bangalore`
- [ ] `/location/india`
- [ ] `/location/remote`

### Legal & Utility
- [ ] `/privacy-policy`
- [ ] `/terms-of-service`
- [ ] `/sitemap` (HTML Sitemap Page)
- [ ] `/thank-you`
- [ ] `/sitemap.xml` (auto-generated)
- [ ] `/robots.txt` (auto-generated)
